...
import POM_repo.windowSwitchPage;
...
prodimg.clickProductlookupLink();
       ...
       int ran1=jutil.getRandomNum();
        ...
        ccp.getCampName(cellvalue1);
     
windowSwitchPage wsp = new windowSwitchPage(driver);
wsp.prdlookupimg();
     
     wutil.windowSwitch(driver,"Products&action" );
     wsp.seachText(cellvalue);
     wsp.enterProdNameInCamp(driver, cellvalue);
     wutil.windowSwitch(driver,"Campaigns&action");
    ccp.clicksaveButton();
    
    campValidation cv=new campValidation(driver);
    cv.campCreationValidation(driver, cellvalue1);
